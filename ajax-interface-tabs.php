...
<html>
  ...
  <body>
    <h1>Read all posts in category</h1>
    <?php
    ...
    <ul class="tabs">
      <?php while($row = $result->fetch_assoc()){ ?>
        <li class="tab" data-cat="<?php echo $row['category_id']; ?>"><?php echo $row['name']; ?></li>
    </ul>
    <?php }//end while ?>
    <?php
      } //end if num_rows
    ...
    <script type="text/javascript">
      ...
      $(document).on({
        ajaxStart: function(){ $('#display-area').addClass('loading');}, ajaxStop: function(){ $('#display-area').removeClass('loading');}
      })
    </script>
  </body>
</html>
